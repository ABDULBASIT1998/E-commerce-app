...
let serviceAccount = require("./ecom-webstore-firebase-adminsdk-3wq1n-288074e269.json");
admin.initializeApp({
...
const bucketName = "ecom-website-clothing-webstore";
...
app.post('/login',(req, res) => {
    ...
    .then(user => {
        if(!user.exists){
            return res.json({'alert': 'log in email address does not exists'})
        } else{
...
app.post('/order', (req, res) => {
    ...
    const mailOptions ={
        from : 'abdulbasitfakir@gmail.com',
        to: email,
        ...
        <body>
            <div>
                <h1 class="heading">Dear ${email.split('@'),[0]},
                    <span>
    ...
    db.collection('order').doc(docName).set(req.body).then(data => {
        ...
        transporter.sendMail(mailOptions, (err, info) => {
            if(err){
                res.json({'alert':'oops! its seeems like some error occured. Try again'});
